...
export async function seed_users(
 ...
 const head_TeacherService = new HeadTeacherService(prisma);
  ...
  // ================= TEACHER =================
  const isHeadTeacher = headTeacherAccounts.includes(userData.account);
  ...
  where: { id: newUser.id_us },
});

if (isHeadTeacher && teacher) {
  await head_TeacherService.create({ id: teacher.id });
}
  console.log(
    `✅ Usuario creado: ${newUser.name} (TEACHER${isHeadTeacher ? ' 👑' : ''})`,
  );
}
}
